...
The `v2.7.1.2` release backports v2.8.0's six pollutant concentration sensor types for each region. All 30 new sensors are disabled by default; enable the ones you want in Settings > Devices & Services. The existing entity names and device grouping stay as they were in `v2.7.1.1`. The separate `backport-entity-naming` branch adapts v2.8.0's naming changes for HA 2024.12 testing.
